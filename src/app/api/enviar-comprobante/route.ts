...
export async function POST(req: Request) {
  const { emailDestino, contenidoVenta, contenidoCambio, nombreVenta, nombreCambio } = await req.json()
  ...
  const generarPDF = async (contenido: string, mensajeFinal: string) => {
    ...
    const drawLine = () => {
      y -= 12
      page.drawText('--------------------------------', { x: 20, y, size: 8, font })
    }
  ...
  const pdfCambio = contenidoCambio
    ? await generarPDF(contenidoCambio, 'Este ticket es válido solo para cambio.')
    : null
